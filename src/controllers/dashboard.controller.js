...
async function getFacultySummary(req) {
  const actingUser = await User.findById(req.user.userId).populate(
    "department",
    "name code"
  );
  ...
  if (!actingUser?.department) {
    return {
      ...
      currentTerm,
      department: null,
      stats: { publishedSessionsCount: 0, totalPublishedExams: 0, nextExam: null },
    };
  }
  ...
  const scheduleIds = publishedSchedules.map((s) => s._id);

  const exams = await Exam.find({ examSchedule: { $in: scheduleIds } })
  ...
  const nextExam = exams
    .filter((exam) => exam.timeSlot?.date && exam.timeSlot.date >= today)
    .sort((a, b) =>
      `${a.timeSlot.date} ${a.timeSlot.startTime}`.localeCompare(
        `${b.timeSlot.date} ${b.timeSlot.startTime}`
      )
    )[0];
  ...
  return {
    ...
    stats: {
      publishedSessionsCount: publishedSchedules.length,
      ...
      nextExam: nextExam
        ? {
            course: nextExam.course
              ...
              : "Unknown course",
            date: nextExam.timeSlot.date,
            startTime: nextExam.timeSlot.startTime,
            endTime: nextExam.timeSlot.endTime,
          }
        : null,
    },
  };
...
async function getSummary(req, res) {
  try {
    ...
    if (req.user.role === "Faculty") {
      return res.json(await getFacultySummary(req));
    }
